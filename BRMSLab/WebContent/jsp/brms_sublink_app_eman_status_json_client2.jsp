...
<%
   ...
   if (dom_json != null && app_json == null) {
	   ...
	   webPage = "http://localhost:8085/J2EESampleProject/status";
   }
   String name = "brm.gen";
		...
		while (lcKeys.hasNext()) {
			...
			if ( lc.get(lifecycle) instanceof JSONObject ) {
				...
				while (domainKeys.hasNext()) {
					...
					//String bgColor = "red";
					if(domainAllChildJson.has("STATUS") && domainAllChildJson.has("APPS") ) {
						domainChildMonStatus = domainAllChildJson.get("STATUS").toString();
						String domainURL = currentURL+"?dom="+domain;
				%>
						<tr>
						<td><%=lifecycle.toUpperCase()%></td>
						<td><a href=<%=domainURL%> ><%=domain%></a></td>
						<td bgcolor=""><i><a href=<%=domainChildProxyUrl%>> <%=domainChildMonStatus%></a></i></td></tr>
				<%
					} else if ( (domainAllChildJson.has("STATUS") && !domainAllChildJson.has("APPS"))) {
				%>
						<tr>
						<td><%=lifecycle.toUpperCase()%></td>
						<td><%=domain%></td>
