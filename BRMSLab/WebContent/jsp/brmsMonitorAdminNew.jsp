...
if(!userID.equalsIgnoreCase("sumkuma2")){
...
TreeMap<String,String> sortedHostMaskEntries= new TreeMap<String,String>();
if(maskFile.exists()){
		...
		if(null==entry  || entry.trim().equalsIgnoreCase("")){
			...
			if(entries[1].equalsIgnoreCase(domainApp)){
				if(entries.length==2) {
					...
					allDisable=true;
				} else if( entries.length == 6 && (entries[2].equalsIgnoreCase("all")) && (entries[3].equalsIgnoreCase("active")) ) {
					allDisable=true;
				} else if( entries.length == 6 && (entries[2].equalsIgnoreCase("all")) && (entries[3].equalsIgnoreCase("inactive")) ) {
					allDisable=true;
					allInactive=true;
				} else {
					if( entries.length == 6) {
						sortedMaskEntries.put(entries[2], true);
						//System.out.println("ENTEORIESSSSSSSSSSSSSSSS keyyyyy::"+entries[2]);
						//System.out.println("ENTEORIESSSSSSSSSSSSSSSS valuuuuue::"+entries[3]+"|"+entries[4]+"|"+entries[5]);
						sortedHostMaskEntries.put(entries[2],entries[3]+"|"+entries[4]+"|"+entries[5]);
						
					} else {
						System.out.println("ENTEORIES::"+entries[2]);
						sortedMaskEntries.put(entries[2], true);
					}
				}
			} else if(!vdc.equalsIgnoreCase("None") && !vdcHost.equalsIgnoreCase("None")){
				System.out.println("NETRIES LENGTH in VDC::"+entries.length);
				if(entries.length == 3 && entries[2].contains(vdcHost)) {
					sortedMaskEntries.put(entries[2]+":"+entries[1], true);
				} else if (entries.length == 6 && entries[2].contains(vdcHost)) {
					System.out.println("ENTEORIESSSSSSSSSSSSSSSSSS::"+entries[2]+":"+entries[1]);
					sortedMaskEntries.put(entries[2]+":"+entries[1], true);
					sortedHostMaskEntries.put(entries[2]+":"+entries[1],entries[3]+"|"+entries[4]+"|"+entries[5]);
...
<html>
	<head>
		...
		function updateEnableDisableMonDetails(path){
			...
			var allInactive;
			if(document.getElementById('selectAllInactiveHost').checked)
				allInactive='true';
			else
				allInactive='false';
			
			document.getElementById("formAction").value="UPDATE_ENABLE_DISABLE_MONITOR";
			document.brmsMonitorAdminform.action = path+"/monitorConfig?view=DisableEnableAlert&allDisable="+allDisable+"&allInactive="+allInactive+"&env="+env+"&domainApp="+domainApp+"&vdc="+vdc+"&vdcHost="+vdcHost;
			document.brmsMonitorAdminform.submit();
		}
		...
		function filterOnDomainEnv(path){
			...
			var env=document.getElementById('selectEnv').value;
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=AlertConfig&domainApp="+domain+"&env="+env;
			document.brmsMonitorAdminform.submit();
		}
		...
		function filterOnDomainEnvED(path){
			...
			var env=document.getElementById('selectEnvForED').value;
			var vdc=document.getElementById('selectVDCForED').value;
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=DisableEnableAlert&domainApp="+domain+"&env="+env;
			document.brmsMonitorAdminform.submit();
		}
		...
		function filterOnVDCEnvED(path){
			var env=document.getElementById('selectEnvForED').value;
			var vdc=document.getElementById('selectVDCForED').value;
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=DisableEnableAlert&domainApp=All&env="+env+"&vdc="+vdc;
			document.brmsMonitorAdminform.submit();
		}
		...
		function filterOnHostED(path){
			...
			var vdc=document.getElementById('selectVDCForED').value;
			var vdcHost=document.getElementById('selectVDCHostForED').value;
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=DisableEnableAlert&domainApp=All&env="+env+"&vdc="+vdc+"&vdcHost="+vdcHost;
			document.brmsMonitorAdminform.submit();
		}
		
		function resetForm(path){
			var env=document.getElementById('selectEnv').value;
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=AlertConfig&env="+env;
			document.brmsMonitorAdminform.submit();
		}
		
		function resetFormED(path){
			var env=document.getElementById('selectEnvForED').value;
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=DisableEnableAlert&env="+env;
			document.brmsMonitorAdminform.submit();
		}
		
		function LoadMonitorDet(path){
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=MonitorConfig";
			document.brmsMonitorAdminform.submit();
		}
		
		function LoadAlertDet(path){
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=AlertConfig";
			document.brmsMonitorAdminform.submit();
		}
		
		function LoadEnableDisableDet(path){
			document.brmsMonitorAdminform.action = path + "/jsp/brmsMonitorAdminNew.jsp?view=DisableEnableAlert";
			document.brmsMonitorAdminform.submit();
		}
		...
		function onWindowLoad(){
			...
			if(vdc.toUpperCase()!='None'.toUpperCase() && !vdcHost.toUpperCase()!='None'.toUpperCase()){
				...
				document.getElementById('selectAllInactiveHost').disabled="disabled";
			}
		}
	...
	<body onload="onWindowLoad()">
		...
		<div id="MonitorConfigDiv" style="display: none;">
			...
			<table border="1" cellspacing="2" cellpadding="2" bgcolor="#cee7ff" class="brmsTable" width="100%">
				...
				<td colspan="5" align="center">
					<input type="submit" id="modifyMonitorConfig" onclick="updateMonitorConfig('<%= strURLPath %>/monitorConfigNew');" value="Update"/>
		...
		<div id="DisableEnableAlertDiv" style="display: none;">
			...
			<table border="1" cellspacing="2" cellpadding="2" bgcolor="#cee7ff" class="brmsTable" width="100%">
				...
				<th colspan="1"  align="left">Make Domain Inactive : &nbsp;
					<input type="checkbox" id="selectAllInactiveHost" style="display: block;"  name="selectAllInactiveHost" <%=allInactive ? "checked" : "" %> />
				</th>
				</tr>
				<% if(sortedMaskEntries.size()>0) { 
					if(!domainApp.equalsIgnoreCase("All")){ %>
					<tr><th>HOST NAME</th><th>PORT</th><th>Mask Individual Nodes : &nbsp;</th><th>Make Nodes Inactive : &nbsp;</th></tr>
					<%}else{ %>
					<tr><th>HOST NAME:PORT</th><th>Domain</th><th>Mask Individual Nodes : &nbsp;</th><th>Make Nodes Inactive : &nbsp;</th></tr>
					<%}
					for(Map.Entry<String,Boolean> entry : sortedMaskEntries.entrySet()) {
						System.out.println("KEYYYYYYy :"+entry.getKey());
						System.out.println("Valueeee :"+entry.getValue());
						String[] tokens = entry.getKey().split(":");
						
						if(tokens.length == 2) {
							System.out.println("TOEKN LENTH IN JSP :"+tokens[1]+"AND"+tokens[0]);
						%>						
						<tr>
							<td><%=tokens[0]%></td>
							<td><%=tokens[1]%> </td>
							<td>
								<input type="checkbox" name="individualHost" value='<%=env+"|"+domainApp+"|"+tokens[0]+":"+tokens[1]+"|Active|"+userID+"|"+new Date()%>' <%=entry.getValue() ? "checked" : "" %> />
							</td>
							<% 
								if(sortedHostMaskEntries.containsKey(tokens[0]+":"+tokens[1]))  {
									String[] inactiveHostValues = sortedHostMaskEntries.get(tokens[0]+":"+tokens[1]).split("\\|");
									String status = inactiveHostValues[0];
									System.out.println("STATUS:::::::"+status);	
									String username = inactiveHostValues[1];
									String updatedDate =  inactiveHostValues[2];
									
									if (status.equalsIgnoreCase("inactive")) {
									%>
										<td>
											<input type="checkbox" name="individualInactiveHost" value='<%=env+"|"+domainApp+"|"+tokens[0]+":"+tokens[1]+"|"+status+"|"+username+"|"+updatedDate%>' checked/>
										</td>
									<%}  else { %>
										<td>
											<input type="checkbox" name="individualInactiveHost" value='<%=env+"|"+domainApp+"|"+tokens[0]+":"+tokens[1]+"|Inactive|"+userID+"|"+new Date()%>'/>
										</td>					
								<%}%>
						...
						<%} else {
							System.out.println("TOEKN LENTH IN JSPPPPP :"+tokens.length);
						%>
							...
							<td><%=tokens[2]%> </td>
							<td>
								<input type="checkbox" name="individualHost" value='<%=env+"|"+tokens[2]+"|"+tokens[0]+":"+tokens[1]+"|Active|"+userID+"|"+new Date()%>' <%=entry.getValue() ? "checked" : "" %> />
							</td>
							<% 
								if(sortedHostMaskEntries.containsKey(tokens[0]+":"+tokens[1]+":"+tokens[2]))  {
									String[] inactiveHostValues = sortedHostMaskEntries.get(tokens[0]+":"+tokens[1]+":"+tokens[2]).split("\\|");
									String status = inactiveHostValues[0];
									System.out.println("STATUS:::::::"+status);	
									String username = inactiveHostValues[1];
									String updatedDate =  inactiveHostValues[2];
									...
									%>
										<td>
											<input type="checkbox" name="individualInactiveHost" value='<%=env+"|"+tokens[2]+"|"+tokens[0]+":"+tokens[1]+"|"+status+"|"+username+"|"+updatedDate%>' checked/>
										</td>
									<%}  else { %>
										<td>
											<input type="checkbox" name="individualInactiveHost" value='<%=env+"|"+tokens[2]+"|"+tokens[0]+":"+tokens[1]+"|Inactive|"+userID+"|"+new Date()%>'/>
										</td>					
								<%}
							} else {%>
								<td>
				...
				<tr>
					<td colspan="5" align="center"><b><i>*Select server Disable/Enable/Inactive reason Comment:(Required)</i></b>&nbsp;&nbsp;&nbsp;&nbsp;
					
						<select id="comments" name="comments" onchange="enableSubmit();">
							<option value="None">--Select Comment--</option>
							<option value="code deployment">Code Deployment</option>
							<option value="weekly maintenance">Weekly Maintenance</option>
							<option value="infra changes">Infra Changes</option>
							<option value="db issue">DB Issue</option>
							<option value="host reboot">Host Reboot</option>
							<option value="host crash">Host Crash</option>
				...
				<tr>
				<td colspan="5" align="center">
					<input type="button" id="modifyMonitorConfig" style="display: none" onclick="updateEnableDisableMonDetails('<%= strURLPath %>');" value="Update"/>
				</td>
				</tr>
			</table>
			</fieldset>
