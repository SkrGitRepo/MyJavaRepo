...
<%
			...
			boolean isReInstallStartAdmin = UserUtil.isUserValid(userId, PropertyLoader.getInstance().getProperty("reinstall_start_admin"));
			int pageNum=0;
...
<%
/****************************************************************/
	String monDirLoc = PropertyLoader.getInstance().getProperty("monitor_config_base");
...
<html>
	<head>
		...
		<script>
			...
			function filterOnDomainEnv(path){
				...
				document.pegaHealthForm.action = path + "/jsp/pegaHealth.jsp?domainApp="+domain+"&env="+env+"&page=1";
	...
	<body onLoad="onWindowLoad();">
		...
		<%
			...
			%>
				<a href="<%=strURLPath %>/jsp/pegaHealth.jsp?domainApp=<%=domainApp%>&env=<%=env%>&page=<%=j%>"<%if(j==pageNum){%> style="color:red;"<%}%>><%=(j==1) ? "First" : (j==totalPages) ? "Last" : j %>&nbsp;</a>
				<% 	}%>
				<a href="<%=strURLPath %>/jsp/pegaHealth.jsp?domainApp=<%=domainApp%>&env=<%=env%>&page=<%= (pageNum>1) ? pageNum-1 : 1 %>"> &lt;&lt;Prev&nbsp;</a>
				<a href="<%=strURLPath %>/jsp/pegaHealth.jsp?domainApp=<%=domainApp%>&env=<%=env%>&page=<%= (pageNum<totalPages) ? pageNum+1 : totalPages %>"> Next&gt;&gt;&nbsp;</a>
				<a href="<%=strURLPath %>/jsp/pegaHealth.jsp?domainApp=All&env=All&page=1">Pega Health</a>
			</td></tr>
		<% 		}
			} else {
		%>
